...
const inputClass = `
    border border-[#c7cdc9] rounded px-2 py-1 text-sm bg-white
...
const ElementPreview = ({ el }: { el: ElementRef }) => (
    <div className='mt-1 flex items-center gap-1.5 text-xs'>
        <span className='px-1.5 py-0.5 rounded bg-[#dcf3e4] text-[#166534] font-mono border border-[#b8e2c6]'>{el.tag}</span>
                ...
                {draftConfig.category === 'list' && (
                    <div className='flex flex-col gap-1'>
                        ...
                        {draftConfig.fields.map(f => (
                            <div key={f.id} className='flex items-center gap-2 text-xs bg-white border border-[#c7cdc9] rounded px-2 py-1 shadow-[inset_0_1px_2px_rgba(0,0,0,0.06)]'>
                                <span className='font-semibold'>{f.name}</span>
                                <span className='px-1 py-0.5 rounded bg-[#dcf3e4] text-[#166534] font-mono border border-[#b8e2c6]'>{f.ref.tag}</span>
                    ...
                    {functions.map(fn => (
                        ...
                        >
                            <div className='flex items-center gap-2'>
                                ...
                                <span className='text-xs px-1.5 py-0.5 rounded bg-[#dcf3e4] text-[#166534] border border-[#b8e2c6]'>{CATEGORY_LABELS[fn.config.category]}</span>
